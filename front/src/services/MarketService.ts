...
const MarketService = {
    getCardInstanceBuyable : async (): Promise<> => {
        try {
            ...
            const request = new Request('http://localhost:10002/marketPlace', {
                method: 'GET',
    ...
    buyCardInstanceBuyable : async (id: number): Promise<any> => {
        try {
            ...
            const token = localStorage.getItem('auth');
            const bearerToken = 'Bearer '+ token;
            const url = 'http://localhost:10002/marketPlace/buy/'+id
    ...
    sellCardInstance : async (id: number): Promise<any> => {
        ...
        try {
            ...
            const token = localStorage.getItem('auth');
            const bearerToken = 'Bearer '+ token;
            const url = `http://localhost:10002/marketPlace/sell/${id}`;
